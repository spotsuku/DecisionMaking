 ...
 * 書き出しの会話。
 *
 * ここは指示を最小限にする。制約を積むほど、モデルは全部を満たす安全な型を
 * 1つ見つけて繰り返すようになる ──「〜とのことですが、〜ありますか?」が
 * 4ターン続いたのは、細かく形を指定した結果だった。
 * 残すのは、外すとプロダクトが壊れるものだけ。
 */
export function brainstormPrompt(req: BrainstormRequest) {
  const log = req.turns
    ...
    .join("\n");
  return {
    system: `決めきれずにいることを話す人の、聞き役です。自然に会話してください。
